...
void FDupdate(long* TYPE, long* type2node, const double RHO, const double THETA, long k, long* ntype, long* nbranch, long* nanc, double* m, double* logw, tsk_table_collection_t* tables, int *rec_num, int *mut_num, int* coal_num, int* nonrec, long* mut_by_site, long* mut_site, long* mut_state, long* mut_node)
{
	...
	char state[2];
	//double* prob0, p_common0, probtemp0, probtemp20, *lweights0; /*for checking scaling*/
	...
	//prob0 = dou_vec_init(K * L + L - 1 + TYPE_MAX);
	//lweights0 = dou_vec_init(K * L + L - 1 + TYPE_MAX);
    ...
    //p_common0 = CSDapprx(TYPE, k, newseq, *nbranch, *ntype, nanc, RHO);
    ...
    for(i=0; i<L; i++){
        ...
        if((*(TYPE+(L+1)*k+i) > 0) && (*(mut_by_site+i) == 1)){
            ...
            for(j=0; j<K; j++){
                if(P[j][a]>0){
					if (j == a) {
						//prob0[K * i + j] = THETA * p_common0 * P[j][a];
						//lweights0[K * i + j] = 0;
						...
						//probtemp0 = CSDapprx(TYPE, k, newseq, *nbranch, *ntype, nanc, RHO);
						probtemp = CSDapprx_scaling(TYPE, k, newseq, *nbranch, *ntype, nanc, RHO);
						newseq[i] = a;
						
						//lweights0[K * i + j] = log(p_common0) - log(probtemp0);
						//prob0[K * i + j] = THETA * probtemp0 * P[j][a];
    ...
    for(i=0; i<*ntype; i++){
        ...
        if(i!=k){
            ...
            if(j==L){
				...
				*nbranch -= 1;
				if (e == 1 && i < k) {
					//probtemp0 = CSDapprx(TYPE_st, k-1, newseq, *nbranch, *ntype, nanc, RHO); /*p(gamma | H-alpha-beta)*/
					//probtemp20 = CSDapprx(TYPE_st, k-1, old, *nbranch, *ntype, nanc, RHO); /*p(beta | H-alpha-beta)*/
					...
					//probtemp0 = CSDapprx(TYPE_st, k, newseq, *nbranch, *ntype, nanc, RHO); /*p(gamma | H-alpha-beta)*/
					//probtemp20 = CSDapprx(TYPE_st, k, old, *nbranch, *ntype, nanc, RHO); /*p(beta | H-alpha-beta)*/
				...
				*nbranch += 1;
				...
				//prob0[K * L + L - 1 + i] = *(TYPE + (L + 1) * i + L) * probtemp0 / probtemp20;
				//lweights0[K*L+L-1+i] = log(p_common0) + log(probtemp20) - log(probtemp0);
		...
		/*have to decide if there is more than one type k*/
			...
			//prob0[K * L + L - 1 + i] = *(TYPE + (L + 1) * i + L) - 1.0;
			//lweights0[K * L + L - 1 + i] = log(p_common0) + log((*(TYPE + (L + 1) * i + L) == 1) ? 0 : 1);
	...
	//if((*(prob0 + e - 1) > 0) && (*(lweights0 + e - 1) != -INFINITY)){
	if ((*(lprob + e - 1) != -INFINITY) && (*(lweights + e - 1) != -INFINITY)) {
         *logw += *(lweights + e - 1);
		//fprintf(stderr, "\n scaling = %lf, no scaling = %lf\n", *(lweights + e - 1), *(lweights0 + e - 1));
    }
